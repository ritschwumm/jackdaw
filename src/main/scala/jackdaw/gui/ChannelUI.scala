...
final class ChannelUI(strip:Strip, tone:Option[Tone], peak:Signal[Float], phoneEnabled:Boolean, keyboard:Signal[Set[Key]], keyTarget:Signal[Boolean]) extends UI with Observing {
	...
	private val	panel	=
		GridBagUI(
			delayedToneUI	.gbi.pos(0,0) .size(1,1) .weight(1,0) .fill("NONE")		.anchor("CENTER")	.insetsTLBR(0,0,6,0),
			delayedStripUI	.gbi.pos(0,1) .size(1,1) .weight(1,1) .fill("VERTICAL")	.anchor("EAST")		.insetsTLBR(6,0,0,0)
		)
