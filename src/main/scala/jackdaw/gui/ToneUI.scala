...
final class ToneUI(tone:Tone, keyInput:KeyInput) extends UI with Observing {
	...
	private val trimKnob	= UIFactory.trimRotary		(tone.trim.signal)
	private val filterKnob	= UIFactory.filterRotary	(tone.filter.signal)
	private val highKnob	= UIFactory.trimRotary		(tone.high.signal)
	private val midKnob		= UIFactory.trimRotary		(tone.middle.signal)
	private val lowKnob		= UIFactory.trimRotary		(tone.low.signal)
	...
	private val panel	=
		GridBagUI(
			trimKnob	.gbi.pos(0,0) .size(1,1) .weight(1,1) .fill("NONE") .insetsTLBR(0,0,2,0),
			filterKnob	.gbi.pos(0,1) .size(1,1) .weight(1,1) .fill("NONE") .insetsTLBR(2,0,8+2,0),
			highKnob	.gbi.pos(0,2) .size(1,1) .weight(1,1) .fill("NONE") .insetsTLBR(2,0,2,0),
			midKnob		.gbi.pos(0,3) .size(1,1) .weight(1,1) .fill("NONE") .insetsTLBR(2,0,2,0),
			lowKnob		.gbi.pos(0,4) .size(1,1) .weight(1,1) .fill("NONE") .insetsTLBR(2,0,0,0)
		)
