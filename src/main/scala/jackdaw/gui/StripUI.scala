...
final class StripUI(strip:Strip, peak:Signal[Float], phoneEnabled:Boolean, keyInput:KeyInput) extends UI with Observing {
	...
	private val speakerUI	= UIFactory.volumeLinear(strip.speaker.signal)
	private val phoneUI		= UIFactory.volumeRotary(strip.phone.signal)
	...
	private val	panel	=
		GridBagUI(
			speakerUI	.gbi.pos(0,1) .size(1,1) .weight(1,1) .fill("VERTICAL")	.anchor("EAST")		.insetsTLBR(0,0,2,2),
			meterUI		.gbi.pos(1,1) .size(1,1) .weight(1,1) .fill("VERTICAL")	.anchor("WEST")		.insetsTLBR(0+z,2,2+z,0),
			phoneUI		.gbi.pos(0,2) .size(2,1) .weight(1,0) .fill("NONE")		.anchor("CENTER")	.insetsTLBR(2,0,0,0)
		)
